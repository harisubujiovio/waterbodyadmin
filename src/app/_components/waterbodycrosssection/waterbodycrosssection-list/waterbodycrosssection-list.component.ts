...
@Component({
  selector: 'app-waterbodycrosssection-list',
  templateUrl: './waterbodycrosssection-list.component.html',
  styleUrls: ['./waterbodycrosssection-list.component.css']
})
export class WaterbodycrosssectionListComponent implements OnInit {
  ...
  constructor(private waterbodycrosssectionService: WaterbodycrosssectionService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodycrosssectionService);
    ...
    ngAfterViewInit() {
      ...
      merge(this.sort.sortChange, this.paginator.page)
        .pipe(
          tap(() => this.BindWaterbodycrosssections())
        )
        .subscribe();
    }
    applyFilter(event: any) {
      ...
      this.BindWaterbodycrosssections();
    }
    BindWaterbodycrosssections() {
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodycrosssectionService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodycrosssections();
            },
