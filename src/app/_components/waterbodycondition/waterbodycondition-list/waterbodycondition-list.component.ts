...
@Component({
  selector: 'app-waterbodycondition-list',
  templateUrl: './waterbodycondition-list.component.html',
  styleUrls: ['./waterbodycondition-list.component.css']
})
export class WaterbodyconditionListComponent implements OnInit {
  ...
  constructor(private waterbodyconditionService: WaterbodyconditionService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodyconditionService);
    ...
    ngAfterViewInit() {
      ...
      merge(this.sort.sortChange, this.paginator.page)
        .pipe(
          tap(() => this.BindWaterbodyConditions())
        )
        .subscribe();
    }
    applyFilter(event: any) {
      ...
      this.BindWaterbodyConditions();
    }
    BindWaterbodyConditions() {
    ...
    public redirectToDetails = (id: string) => {
      let url: string = `/waterbodycondition/details/${id}`;
      this.router.navigate([url]);
    }
    public redirectToUpdate = (id: string) => {
      let url: string = `/waterbodycondition/edit/${id}`;
      this.router.navigate([url]);
    }
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodyconditionService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodyConditions();
            },
