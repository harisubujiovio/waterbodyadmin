...
@Component({
  selector: 'app-waterbodytype-list',
  templateUrl: './waterbodytype-list.component.html',
  styleUrls: ['./waterbodytype-list.component.css']
})
export class WaterbodytypeListComponent implements OnInit {
  ...
  constructor(private waterbodytypeService: WaterbodytypeService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodytypeService);
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodytypeService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodytypes();
            },
