...
@Component({
  selector: 'app-waterbodysluice-list',
  templateUrl: './waterbodysluice-list.component.html',
  styleUrls: ['./waterbodysluice-list.component.css']
})
export class WaterbodysluiceListComponent implements OnInit {
  ...
  constructor(private waterbodysluiceService: WaterbodysluiceService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodysluiceService);
    ...
    applyFilter(event: any) {
      ...
      this.BindWaterbodySluices();
    }
    BindWaterbodySluices() {
    ...
    public redirectToDetails = (id: string) => {
      let url: string = `/waterbodysluice/details/${id}`;
      this.router.navigate([url]);
    }
    public redirectToUpdate = (id: string) => {
      let url: string = `/waterbodysluice/edit/${id}`;
      this.router.navigate([url]);
    }
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodysluiceService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodySluices();
            },
