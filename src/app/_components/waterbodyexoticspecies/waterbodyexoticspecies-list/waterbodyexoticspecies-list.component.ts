...
@Component({
  selector: 'app-waterbodyexoticspecies-list',
  templateUrl: './waterbodyexoticspecies-list.component.html',
  styleUrls: ['./waterbodyexoticspecies-list.component.css']
})
export class WaterbodyexoticspeciesListComponent implements OnInit {
  ...
  constructor(private waterbodyexoticspeciesService: WaterbodyexoticspeciesService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodyexoticspeciesService);
    ...
    ngAfterViewInit() {
      ...
      merge(this.sort.sortChange, this.paginator.page)
        .pipe(
          tap(() => this.BindWaterbodyexoticspecies())
        )
        .subscribe();
    }
    applyFilter(event: any) {
      ...
      this.BindWaterbodyexoticspecies();
    }
    BindWaterbodyexoticspecies() {
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodyexoticspeciesService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodyexoticspecies();
            },
