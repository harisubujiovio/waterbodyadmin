...
@Component({
  selector: 'app-waterbodytankissue-list',
  templateUrl: './waterbodytankissue-list.component.html',
  styleUrls: ['./waterbodytankissue-list.component.css']
})
export class WaterbodytankissueListComponent implements OnInit {
  ...
  constructor(private waterbodytankissueService: WaterbodytankissueService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodytankissueService);
    ...
    ngAfterViewInit() {
      ...
      merge(this.sort.sortChange, this.paginator.page)
        .pipe(
          tap(() => this.BindWaterbodyTankIssues())
        )
        .subscribe();
    }
    applyFilter(event: any) {
      ...
      this.BindWaterbodyTankIssues();
    }
    BindWaterbodyTankIssues() {
    ...
    public redirectToDetails = (id: string) => {
      let url: string = `/waterbodytankissue/details/${id}`;
      this.router.navigate([url]);
    }
    public redirectToUpdate = (id: string) => {
      let url: string = `/waterbodytankissue/edit/${id}`;
      this.router.navigate([url]);
    }
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodytankissueService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodyTankIssues();
            },
