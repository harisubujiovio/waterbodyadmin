...
@Component({
  selector: 'app-waterbodyownership-list',
  templateUrl: './waterbodyownership-list.component.html',
  styleUrls: ['./waterbodyownership-list.component.css']
})
export class WaterbodyownershipListComponent implements OnInit {
  ...
  constructor(private waterbodytypeownershipService: WaterbodytypeownershipService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodytypeownershipService);
    ...
    ngAfterViewInit() {
      ...
      merge(this.sort.sortChange, this.paginator.page)
        .pipe(
          tap(() => this.BindWaterbodyownerships())
        )
        .subscribe();
    }
    applyFilter(event: any) {
      ...
      this.BindWaterbodyownerships();
    }
    BindWaterbodyownerships() {
    ...
    public redirectToDetails = (id: string) => {
      let url: string = `/waterbodyownership/details/${id}`;
      this.router.navigate([url]);
    }
    public redirectToUpdate = (id: string) => {
      let url: string = `/waterbodyownership/edit/${id}`;
      this.router.navigate([url]);
    }
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodytypeownershipService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodyownerships();
            },
