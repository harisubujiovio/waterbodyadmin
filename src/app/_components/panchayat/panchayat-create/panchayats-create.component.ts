...
@Component({
  selector: 'app-panchayats-create',
  templateUrl: './panchayats-create.component.html',
  styleUrls: ['./panchayats-create.component.css']
})
export class PanchayatsCreateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.blockService.getDictionary()
    .subscribe(response => {
  ...
  ngOnInit(): void {
    ...
    this.formTitle = this.isAddMode ? 'Add Panchayt' : 'Edit Panchayt'
    ...
    if (!this.isAddMode) {
      this.panchayatService.get(this.id)
        .pipe(first())
  ...
  private createResource() {
    ...
    this.panchayatService.create(newresource)
      .pipe(first())
      .subscribe({
        next: () => {
  ...
  private updateResource() {
    ...
    this.panchayatService.update(this.id, updatedresource)
      .pipe(first())
      .subscribe({
        next: () => {
          let dialogRef = this.dialog.open(SuccessDialogComponent, this.dialogConfig);
