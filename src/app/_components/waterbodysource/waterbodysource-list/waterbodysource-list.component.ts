...
@Component({
  selector: 'app-waterbodysource-list',
  templateUrl: './waterbodysource-list.component.html',
  styleUrls: ['./waterbodysource-list.component.css']
})
export class WaterbodysourceListComponent implements OnInit {
  ...
  constructor(private waterbodysourceService: WaterbodysourceService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorHandlerService) { }

    ngOnInit(): void {
      this.dataSource = new ResourceDataSource(this.waterbodysourceService);
    ...
    applyFilter(event: any) {
      ...
      this.BindWaterbodysources();
    }
    BindWaterbodysources() {
    ...
    public redirectToDetails = (id: string) => {
      let url: string = `/waterbodysource/details/${id}`;
      this.router.navigate([url]);
    }
    public redirectToUpdate = (id: string) => {
      let url: string = `/waterbodysource/edit/${id}`;
      this.router.navigate([url]);
    }
    ...
    public delete = (id: string) => {
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          this.waterbodysourceService.delete(id).subscribe({
            next: () => {
              this.paginator.pageIndex = 0;
              this.BindWaterbodysources();
            },
